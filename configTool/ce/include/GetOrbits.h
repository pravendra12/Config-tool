...
/**
 * @brief Get the Orbit map from given equivalent sites under a given symmetry operation
 * 
 * @param config 
 * @param maxClusterSize 
 * @param maxBondOrderOfClusters 
 * @param equivalentSitesVector 
 * @return map<string, vector<vector<size_t>>> 
 */
map<string, vector<vector<size_t>>> GetOrbits(
    const Config &config, 
    const size_t &maxClusterSize, 
