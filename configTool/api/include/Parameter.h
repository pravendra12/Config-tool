...
struct Parameter {
 public:
  ...
  /// Element set for generating the alloy supercell.
  std::vector<std::string> element_vector_{};

  /// Elmenet Composition for generating the alloy supercell.
  std::vector<double> element_composition_{};

  /// Number of Unique Structures
  size_t num_unique_structure_{};

  /// Element pair which will form B2
  std::pair<std::string, std::string> b2_element_pair_{};
  ...
  /// Number of random structures to be generated
  size_t num_random_structures_{};

  /// Maximum number of B2 centers
  size_t max_num_B2_center_{};

  /// Number of samples per config with B2 ordered structure
  size_t num_sample_per_B2_config_{};

  /// Whether to compute energy of the configuration or not
