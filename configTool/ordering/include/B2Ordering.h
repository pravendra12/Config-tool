#ifndef LMC_ORDERING_INCLUDE_B2ORDERING_H_
#define LMC_ORDERING_INCLUDE_B2ORDERING_H_
...
public:
  /**
   * @brief Constructs a B2Ordering object and initializes lattice site classifications.
   *
   ...
   */
  B2Ordering(const Config &config);
   ...
   * \f[
   * f_{\alpha} = \frac{\text{Number of alpha sites occupied by the element}}{\text{Total number of sites}}
   * \f]
   ...
   * @return Fractional occupancy of the element at alpha sites (a value between 0 and 0.5).
   */
   ...
   * \f[
   * f_{\beta} = \frac{\text{Number of beta sites occupied by the element}}{\text{Total number of sites}}
   * \f]
   * It iterates through the beta sites and counts how many are occupied by the given element.
   *
   * @param element The element whose occupancy is being calculated.
   * @return Fractional occupancy of the element at beta sites (a value between 0 and 0.5).
   */
  ...
  /**
   * @brief Adds B2 Precipitate to an existing configuration
   * 
   * @param config Configuration in which B2 Precipitate nucleates
   * @param numB2Centers Number of B2 Centers
   * @param elementVector B2 forming elements
   */
   ...
   *        are assigned to alpha and beta lattice sites in specific order.
   *        Also replicates segregation.
   * 
   * @param config Configuration which will be transformed in place.
   */
  void InitializeB2Structure(Config &config);

  static bool isB2Structure(const Config &config, const size_t latticeId);
...
private:
   ...
   */
  const size_t numLattice = config_.GetNumLattices();
...
struct B2OrderingInfo
{
  // Fraction of sites following B2 ordering
  // double orderingFraction;
  bool isOrderedStructure;

  // Configuration with applied B2 ordering
  Config orderedConfig;

  // Alpha sublattice properties

  // Element assigned to alpha sublattice
  Element element1;
  // Fraction of alpha sites occupied
  double alphaOccupancyFractionElement1;
