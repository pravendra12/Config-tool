...
// Returns the miller index direction
Eigen::Vector3d
Config::GetNormalizedDirection(size_t referenceId, size_t latticeId) const
{
  auto diff = GetRelativePositionOfLattice(referenceId) - GetRelativePositionOfLattice(latticeId);
...
Config Config::GeneratePristineSupercell(size_t supercell_size, double lattice_param, const std::string &element_symbol, const std::string &structure_type)
{
...
VacancyMigrationInfo Config::GenerateNEBStructure(const size_t vacancyId,
                                                  const size_t migratingAtomId,
                                                  std::string folder,
                                                  std::string filename)
{

  auto num_atoms = GetNumAtoms();

  // auto initial_vacancy_id = atom_index_selector_(mersenne_twister_rng_);
  auto initial_vacancy_id = vacancyId;
  auto initial_vacancy_position = GetRelativePositionOfLattice(initial_vacancy_id);

  // auto neighbours_vacancy_id =
  //                    GetNeighborLatticeIdVectorOfLattice(initial_vacancy_id, 1);

  // std::uniform_int_distribution<size_t> neighbour_index_selector_(0, neighbours_vacancy_id.size()-1);

  // auto final_vacancy_id =
  //      neighbours_vacancy_id[neighbour_index_selector_(mersenne_twister_rng_)];
  auto final_vacancy_id = migratingAtomId;

  auto final_vacancy_position = GetRelativePositionOfLattice(final_vacancy_id);

  auto migrating_atom = GetElementOfLattice(final_vacancy_id);

  auto first_nn = GetSortedLatticeVectorStateOfPair({initial_vacancy_id, final_vacancy_id}, 1);
  auto second_nn = GetSortedLatticeVectorStateOfPair({initial_vacancy_id, final_vacancy_id}, 2);
  auto third_nn = GetSortedLatticeVectorStateOfPair({initial_vacancy_id, final_vacancy_id}, 3);

  // Initial NEB Structure
  Eigen::Matrix3Xd relative_position_matrix(3, num_atoms - 1);

  relative_position_matrix << relative_position_matrix_.leftCols(initial_vacancy_id),
      relative_position_matrix_.rightCols(relative_position_matrix_.cols() - initial_vacancy_id - 1);

  auto initial_atom_vector = atom_vector_;

  initial_atom_vector.erase(initial_atom_vector.begin() + initial_vacancy_id);

  Config initial_config = Config{basis_,
                                 relative_position_matrix,
                                 initial_atom_vector};
  initial_config.ReassignLattice();
  initial_config.Wrap();

  WriteLAMMPSDataFile(folder + "/" + filename + "_initial.data", initial_config);

  // Final NEB Structure

  double tolerance = 1e-6;
  for (int i = 0; i < relative_position_matrix.cols(); ++i)
  {
    if ((relative_position_matrix.col(i) - final_vacancy_position).norm() < tolerance)
    {
      relative_position_matrix.col(i) = initial_vacancy_position; // Move the neighbouring atom to the vacancy
      break;
    }
  }

  Config final_config = Config{basis_,
                               relative_position_matrix,
                               initial_atom_vector};
  final_config.ReassignLattice();
  final_config.Wrap();

  WriteLAMMPSDataFile(folder + "/" + filename + "_final.data", final_config);

  VacancyMigrationInfo info(1,
                            "materialSystem",
                            initial_vacancy_id,
                            final_vacancy_id,
                            GetCartesianPositionOfLattice(initial_vacancy_id),
                            GetCartesianPositionOfLattice(final_vacancy_id),
                            migrating_atom,
                            first_nn,
                            second_nn,
                            third_nn);
...
void Config::WriteVacancyMigrationInfo(const size_t vacancyId,
                                       const size_t migratingAtomId,
                                       const VacancyMigrationInfo &vacancyInfo,
                                       const Config config,
                                       const std::vector<std::string> elementSet,
                                       const std::vector<double> elementComposition,
                                       ...
                                       const std::string predictorFilename)
{
  ...
  size_t maxBondOrder = 2;

  // forward Jump
  // Migrating Atom jumps from 74 to 54
  // finally vacancy at 74
  // first one is the initial id of the migrating atom
  std::pair<size_t, size_t> forwardJumpPair = {migratingAtomId, vacancyId};

  // backward Jump
  // Migrating Atom jumps from 54 to 74
  // finally vacancy at 54
  // first one is the initial id of the migrating atom
  std::pair<size_t, size_t> backwardJumpPair = {vacancyId, migratingAtomId};

  auto atomVector = config.GetAtomVector();
  std::set<Element> setElement(atomVector.begin(), atomVector.end());

  auto encodeVectorForward = GetEncodeVector3F(config,
                                               forwardJumpPair,
                                               setElement,
                                               maxBondOrder);

  auto encodeVectorBackward = GetEncodeVector3F(config,
                                                backwardJumpPair,
                                                setElement,
                                                maxBondOrder);

  auto encodeMigrationAtomPairsForward = GetEncodingMigratingAtomPairs(config,
                                                                       migratingAtomId,
                                                                       forwardJumpPair,
                                                                       setElement,
                                                                       maxBondOrder);

  auto encodeMigrationAtomPairsBackward = GetEncodingMigratingAtomPairs(config,
                                                                        migratingAtomId,
                                                                        backwardJumpPair,
                                                                        setElement,
                                                                        maxBondOrder);
  ...
  // CE for the starting configuration
  ...
  // std::cout << cfg.GetElementOfLattice(54) << std::endl;

  atomVector = cfg.GetAtomVector();

  std::set<Element> elementSetCE(atomVector.begin(), atomVector.end());
  ...
  auto encodeCE = peEstimator.GetEncodeVector(cfg);
  ...
  if (outputFile.is_open())
  {
    ...
    if (outputFile.tellp() == 0)
    {
      ...
      outputFile << "FolderId\tSystem\tInitial Vacancy ID\tInitial Vacancy Position\tFinal Vacancy ID\tFinal Vacancy Position\tMigrating Atom\t";
      outputFile << "First Nearest Neighbors\tSecond Nearest Neighbors\tThird Nearest Neighbors\t"
                 << "3-Fold Rotation Forward Encoding\t"
                 << "3-Fold Rotation Backward Encoding\t"
                 << "Migration Atom Pairs Forward Encoding\t"
                 << "Migration Atom Pairs Backward Encoding\t"
                 << "Cluster Expansion Encoding"
                 << std::endl;
    }
    ...
    for (int i = 0; i < elementSet.size(); ++i)
    {
      if (!first)
      {
        outputFile << "-"; // Add a hyphen before printing the next element
      }
      outputFile << elementSet[i] << std::to_string(int(elementComposition[i]));
      first = false;
    }
    outputFile << "\t";
    outputFile << vacancyInfo.initialVacancyIndex << "\t";
    outputFile << vacancyInfo.initialVacancyPosition << "\t";
    outputFile << vacancyInfo.finalVacancyIndex << "\t";
    outputFile << vacancyInfo.finalVacancyPosition << "\t";
    outputFile << vacancyInfo.migratingAtom << "\t";

    // Writing first_nn list in a cell format (comma-separated values)
    for (auto id : vacancyInfo.firstShellNeighbors)
    {
      outputFile << id << " " << config.GetElementOfLattice(id) << ", ";
    }
    outputFile << "\t"; // Tab to separate columns

    // Writing second_nn list in a cell format
    for (auto id : vacancyInfo.secondShellNeighbors)
    {
      outputFile << id << " " << config.GetElementOfLattice(id) << ", ";
    }
    outputFile << "\t"; // Tab to separate columns

    // Writing third_nn list in a cell format
    for (auto id : vacancyInfo.thirdShellNeighbors)
    {
      outputFile << id << " " << config.GetElementOfLattice(id) << ", ";
    }
    outputFile << "\t"; // Tab to separate columns

    // Writing 3-Fold Rotation Forward Encoding Vector
    for (size_t i = 0; i < encodeVectorForward.size(); ++i)
    {
      if (i == encodeVectorForward.size() - 1)
      {
        outputFile << encodeVectorForward[i];
      }
      else
      {
        outputFile << encodeVectorForward[i] << ", ";
    ...
    // Writing 3-Fold Rotation Backward Encoding Vector

    for (size_t i = 0; i < encodeVectorBackward.size(); ++i)
    {
      if (i == encodeVectorBackward.size() - 1)
      {
        outputFile << encodeVectorBackward[i];
      }
      else
      {
        outputFile << encodeVectorBackward[i] << ", ";
    ...
    for (size_t i = 0; i < encodeMigrationAtomPairsForward.size(); ++i)
    {
      if (i == encodeMigrationAtomPairsForward.size() - 1)
      {
        outputFile << encodeMigrationAtomPairsForward[i];
      }
      else
      {
        outputFile << encodeMigrationAtomPairsForward[i] << ", ";
    ...
    for (size_t i = 0; i < encodeMigrationAtomPairsBackward.size(); ++i)
    {
      if (i == encodeMigrationAtomPairsBackward.size() - 1)
      {
        outputFile << encodeMigrationAtomPairsBackward[i];
      }
      else
      {
        outputFile << encodeMigrationAtomPairsBackward[i] << ", ";
    ...
    for (size_t i = 0; i < encodeCE.size(); ++i)
    {
      if (i == encodeCE.size() - 1)
      {
        outputFile << encodeCE[i];
      }
      else
      {
        outputFile << encodeCE[i] << ", ";
    ...
    const Config &config_out)
{
  ...
  // Assuming Atom objects have a function GetElementString() to return the element's name as a string
  std::set<std::string> element_set;
  for (const auto &atom : atomVector)
  {
    element_set.insert(atom.GetElementString());
  }

  // Assuming 1 atom type, adjust this based on your configuration
  ofs << element_set.size() << " atom types\n\n";
  ...
  {
    const auto &atom = atom_vector[i]; // Assuming atom_vector is accessible via index
    ...
    ofs << (i + 1) << " " << element_map[atom] << " "; // Atom ID and type
