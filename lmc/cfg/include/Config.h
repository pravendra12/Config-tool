...
public:
    ...
    /*! \brief Returns the lattice Id of the central atom.
     *         This function should only be used for small configurations.
     *  \return         The lattice Id of the atom.
     */
    ...
    Eigen::Vector3d GetNormalizedDirection(size_t referenceId, size_t latticeId) const;

    static void RotateLatticeVector(std::unordered_map<size_t, Eigen::RowVector3d> &lattice_id_hashmap,
                                    const Eigen::Matrix3d &rotation_matrix);

    Eigen::Vector3i GetMillerIndices(size_t referenceId, size_t latticeId) const;
    ...
    // Returns a config with the atom at a given lattice Id deleted
    // Used for generating initial and final config for NEB calculation
    Config CreateVacancy(size_t lattice_id,
     ...
     */
    static Config GeneratePristineSupercell(size_t supercell_size, double lattice_param, const std::string &element_symbol, const std::string &structure_type);
    ...
    [[nodiscard]] VacancyMigrationInfo
    GenerateNEBStructure(const size_t vacancyId,
                         const size_t migratingAtomId,
                         std::string folder,
                         std::string filename);

    static void WriteVacancyMigrationInfo(const size_t vacancyId,
                                          const size_t migratingAtomId,
...
private:
    ...
    //  std::optional<std::random_device> random_device_;  // Seeding device
    //
    //  std::optional<std::mt19937> mersenne_twister_rng_;
    //
    //  std::optional<std::uniform_int_distribution<int>> uniform_distribution_;
};
